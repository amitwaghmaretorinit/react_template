...
function AgreementCheckBox(props: any) {
  ...
  return (
    <Flex direction="column" style={{ position: 'relative', width: maxWidth }}>
      ...
      {isLoading && <Flex style={{ width: '100%', height: 400 }} direction={'column'} justifyContent={'center'}>
        <Spin />
      </Flex>}
      {!!containerId && (
        <>
          {
            !isLoading && <Flex direction="row-reverse" style={{ padding: 10, position: 'absolute', right: 0 }}>
          ...
          <PDFFrame
            onScroll={(e) => {
              ...
              if (
                myDiv &&
                Math.round(myDiv.offsetHeight + myDiv.scrollTop) >=
                myDiv.scrollHeight
              ) {
                setIsScrolledDown(true);
              }
            ...
            id={containerId}
            style={{
              overflowY: "scroll", maxWidth: maxWidth,
      ...
      {!isLoading && <>
        <H5Typography style={{ marginTop: 10 }}>
          You must read the entire agreement above before you can agree to the
          terms.
        </H5Typography>
          ...
          <Flex direction="column" top={10}>
            <Checkbox
              ...
              onChange={(e: any) => {
                ...
                setAgreementCheckedDate && setAgreementCheckedDate(e.target.checked ? true : false);
              }}
            />
